...
export function RunButtons({ onDispatched }: { onDispatched: () => void }) {
  ...
  async function run(workflow: string, inputs?: Record<string, string | boolean>) {
    ...
    setBusy(null);
    if ("error" in result) {
      setMsg({ kind: "err", text: `Failed: ${result.error}` });
    } else {
      setMsg({ kind: "ok", text: "Dispatched. The run will appear below shortly." });
  ...
  return (
    <div className="space-y-6">
      ...
      {msg && (
        <p className={`text-sm ${msg.kind === "err" ? "text-red-400" : "text-neutral-300"}`}>
          {msg.text}
        </p>
      )}
    </div>
